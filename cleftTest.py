...
stateList=["h","j","m","Xr","Xs","X_tos","Y_tos","R_tos","X_tof","Y_tof","d","f",
           ...
           "Ca_SL","Cai","V"]
stateListXX=["h","j","m","Xr","Xs","X_tos","Y_tos","R_tos","X_tof","Y_tof","d","f",
...
if test1:
  ...
  s1 = model.rhs(s,0,p)
  s1p=[]
  for i,statei in enumerate(stateList):
...
#p[msplit.parameter_indices("w1")]=-1;
msplit.w1 = 1.0
...
print s2[43:45]
s2p=[]
for i,statei in enumerate(stateListXX):
...
if test1:
  ...
  z = np.arange(np.shape(stateListXX)[0])[bad]
  print stateListXX[z[0]]
  print msplit.state_indices(stateListXX[z[0]])
  print stateListXX[z]
...
if test3:
  ...
  print z
#print msplit.parameter_indices(z[0])
