...
Fjunc_ncx = Fjunc; Fsl_ncx = 1-Fjunc_ncx; %MOUSE
...
distJuncSL = 0.3;  % dist. junc to SL [um]
...
%J_ca_juncsl = 1/1.2134e12; % [L/msec] RABBIT
%J_ca_slmyo = 1/2.68510e11; % [L/msec] RABBIT
%J_na_juncsl = 1/(1.6382e12/3*100); % [L/msec] RABBIT  
%J_na_slmyo = 1/(1.8308e10/3*100);  % [L/msec] RABBIT
...
%Cao = 1.8;  % Extracellular Ca  [mM] RABBIT
Cao = 1;  % Extracellular Ca  [mM] MOUSE
...
%GNa =  16.0;        % [mS/uF] RABBIT
GNa =  10.0;        % [mS/uF] MOUSE
%GNaB = 0.297e-3;    % [mS/uF] RABBIT 
GNaB = 4.5*0.297e-3;    % [mS/uF] MOUSE 
%IbarNaK = 1.90719;     % [uA/uF] RABBIT
IbarNaK = 5;     % [uA/uF] MOUSE
if NaGainFlag == 1,
    GNaB=GNaB*4;
    IbarNaK=IbarNaK*0.9;
end
...
ksat = 0.27;        % [none]  
nu = 0.35;          % [none]
%Kdact = 0.256e-3;   % [mM]  RABBIT
Kdact = 1/2*0.256e-3; % [mM] changed from rabbit
Q10NCX = 1.57;      % [none]
IbarSLCaP = 0.0673; % [uA/uF]
KmPCa = 0.5e-3;     % [mM] 
%GCaB = 2.513e-4;    % [uA/uF] RABBIT
GCaB = 3*2.513e-4; % [uA/uF] changed from rabbit (2.513e-4)
Q10SLCaP = 2.35;    % [none]

% SR flux parameters
Q10SRCaP = 2.6;          % [none]
%Vmax_SRCaP = 2.86e-4;  % [mM/msec] (mmol/L cytosol/msec) RABBIT
Vmax_SRCaP = 1.15*1.15*2.86e-4; % [mM/msec] (mmol/L cytosol/msec) changed MOUSE
%Kmf = 0.246e-3;          % [mM] RABBIT
Kmf = 0.3e-3; % [mM] changed from rabbit (0.246e-3) % from Yang-Saucerman
%Kmr = 1.7;               % [mM]L cytosol RABBIT
...
%fracTnIpo = .0031;  % Derived quantity (TnI_PKAp(baseline)/TnItot) RABBIT
fracTnIpo = 0.062698;  % Derived quantity (TnI_PKAp(baseline)/TnItot) MOUSE
%fPKA_TnI = (1.45-0.45*(1-TnI_PKAp)/(1-fracTnIpo)); RABBIT
...
%% I_kur - IK,slow--THis section has been added in from MOUSE.
...
%% I_kr: Rapidly Activating K Current--Back to RABBIT and Original Code
%gkr = 0.03*sqrt(Ko/5.4); 
...
%% I_to: Transient Outward K Current (slow and fast components) (ABSENT IN MICE)
%xtoss = 1/(1+exp(-(y(39)+3.0)/15)); RABBIT
xtoss = 1/(1+exp(-(y(39)+3.0)/13));
%ytoss = 1/(1+exp((y(39)+33.5)/10)); RABBIT
ytoss = 1/(1+exp((y(39)+48)/5));
rtoss = 1/(1+exp((y(39)+33.5)/10));  % Rto not used in MOUSE model
%tauxtos = 9/(1+exp((y(39)+3.0)/15))+0.5; RABBIT
tauxtos = 0.08+0.7*exp(-((y(39)+25)/30)^2); 

if ItoFlag == 0 %not used
% Shannon Versions
    %tauytos = 3e3/(1+exp((y(39)+60.0)/10))+30; RABBIT
    tauytos = 100+400/(1+exp((y(39)+25)/5)); % MOUSE
    taurtos = 2.8e3/(1+exp((y(39)+60.0)/10))+220; 
elseif ItoFlag == 1 && CKIIflag == 0
    % Grandi Versions
    Py = 182; Pr1 = 8085; Pr2 = 313;                % Normal
  %  tauytos = Py/(1+exp((y(39)+33.5)/10))+1; RABBIT
...
elseif ItoFlag == 1 && CKIIflag == 1
   ...
   % tauytos = Py/(1+exp((y(39)+33.5)/10))+1;
    ...
    taurtos = Pr1/(1+exp((y(39)+33.5)/10))+Pr2;   % MOUSE tau rec -73% 
end
...
I_tos = 0*GtoSlow*y(8)*y(9)*(y(39)-ek); % N0 Itos in MOUSE % [uA/uF]
...
%tauxtof = 3.5*exp(-((y(39)+3)/30)^2)+1.5;       % Version in Grandi Code
%(does not change AP shape) RABBIT
tauxtof = 0.08+0.7*exp(-((y(39)+25)/30)^2); % = tauxtos;
tauytof = 10+32*exp(-((y(39)+55)/16)^2)+8/(1+exp(-(y(39)+60)/8));
%tauytof = 20.0/(1+exp((y(39)+33.5)/10))+20.0; RABBIT
...
%fracLCCbpo = .0328; % Derived quantity - (LCCbp(baseline)/LCCbtot) RABBIT
...
%fpkam2 = 0.1543*LCCa_PKAp - .0043;  % Assumes max phosphorylation results
%in 15% mode 2 channels RABBIT
fracLCCap0 = 0.219577; % Derived MOUSE
frac_fpkam2 = (0.15*fracLCCap0)/(1-fracLCCap0); %MOUSE
fpkam2 = (0.15+frac_fpkam2)*LCCa_PKAp - frac_fpkam2; %MOUSE
fckiim2 = LCC_CKp*.1;               % Assumes max phosphorylation results in 10% mode 2 channels MOUSE
% Sum up total fraction of CKII and PKA-shifted mode 2 channels MOUSE
...
%RABBIT
%I_ncx_sl = Fsl*IbarNCX*Q10NCX^Qpow*Ka_sl*(s1_sl-s2_sl)/s3_sl; RABBIT
...
%fPKA_RyR = RyR_PKAp*1.025 + 0.9750; % 1 with NO ISO RABBIT
...
%fPKA_PLB = (PLB_PKAn/fracPKA_PLBo)*3/4 + 1/4; RABBIT
...
%J_CaB_cytosol = sum(ydot(19:25)); THis is wrong.
...
switch protocol
    ...
    case 'vclamp_ICa', % Based on Kohlhaas (2006), Maier (2003)  
        ...
        V_step = -50;
       % INa_inactT = 50;    % ms RABBIT
        INa_inactT = 30; % ms MOUSE
    ...
    case 'vclamp_ICa_Gain'
        V_hold = -80;
        V_test = 20;%0.05; MOUSE
        %V_test = 0.05; RABBIT
          if t>1e3
              V_test = -10;
          end
        if mod(t,cycleLength)>= 5 &&  mod(t,cycleLength) <= 35%205 RABBIT
            V_clamp = V_test;
        else
            V_clamp = V_hold;
        end
    ...
    case 'IV_SSI_ICa'
        ...
        if mod(t,cycleLength) <= 250
            V_clamp = V_hold1;
        elseif mod(t,cycleLength) > 250 && mod(t,cycleLength) <= 225
            V_clamp = V_condition;
        elseif mod(t,cycleLength) > 2250 && mod(t,cycleLength) <= 22
            V_clamp = V_hold2;
        elseif mod(t,cycleLength) > 2255 && mod(t,cycleLength) <= 24
            V_clamp = V_step;
        else
            V_clamp = V_hold1;
        end
