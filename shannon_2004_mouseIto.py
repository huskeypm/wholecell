# Gotran generated code for the  "shannon_2004_mouseIto" model
...
def monitor_indices(*monitored):
    ...
    monitor_inds = dict([("Vol_Cell", 0), ("Vol_SR", 1), ("Vol_SL", 2),\
        ...
        ("tau_R_tos", 45), ("i_tof", 46), ("X_toss", 47), ("tau_X_tof", 48),\
        ("Y_toss", 49), ("tau_Y_tof", 50), ("i_Cl_Ca", 51), ("i_Clb", 52),\
        ("Q_CaL", 53), ("temp", 54), ("i_CaL_Ca_jct1", 55), ("i_CaL_Na_jct1",\
        56), ("i_CaL_Ca_SL", 57), ("i_CaL_Na_SL", 58), ("i_CaL_K", 59),\
        ("i_CaL", 60), ("fCa_SL", 61), ("fCa_jct1", 62), ("d_infinity", 63),\
        ("tau_d", 64), ("f_infinity", 65), ("tau_f", 66), ("temp_jct1", 67),\
        ("temp_SL", 68), ("Q_NCX", 69), ("Ka_SL", 70), ("Ka_jct1", 71),\
        ("i_NaCa_jct1", 72), ("i_NaCa_SL", 73), ("i_NaCa", 74), ("Q_SLCaP",\
        75), ("i_Cap_jct1", 76), ("i_Cap_SL", 77), ("i_Cap", 78),\
        ("i_Cab_jct1", 79), ("i_Cab_SL", 80), ("i_Cab", 81), ("kCaSR", 82),\
        ("koSRCa", 83), ("kiSRCa", 84), ("RI", 85), ("j_rel_SR", 86),\
        ("j_leak_SR", 87), ("Q_SRCaP", 88), ("j_pump_SR", 89),\
        ("dCalsequestrin", 90), ("dCa_SLB_SL", 91), ("dCa_SLB_jct1", 92),\
        ("dCa_SLHigh_SL", 93), ("dCa_SLHigh_jct1", 94),\
        ("dCa_jct1_tot_bound", 95), ("dCa_SL_tot_bound", 96),\
        ("i_Ca_jct1_tot", 97), ("i_Ca_SL_tot", 98), ("dCa_TroponinC", 99),\
        ("dCa_TroponinC_Ca_Mg", 100), ("dMg_TroponinC_Ca_Mg", 101),\
        ("dCa_Calmodulin", 102), ("dCa_Myosin", 103), ("dMg_Myosin", 104),\
        ("dCa_SRB", 105), ("dCa_cytosol_tot_bound", 106), ("dNa_jct1_buf",\
        107), ("dNa_SL_buf", 108), ("i_Stim", 109), ("E_Na_jct1", 110),\
        ("E_Na_SL", 111), ("E_Ca_jct1", 112), ("E_Ca_SL", 113), ("E_K", 114),\
        ("E_Cl", 115), ("G_K1", 116), ("i_K1", 117), ("alpha_K1", 118),\
        ("beta_K1", 119), ("K1_infinity", 120), ("J_Na_jct1_SL", 121),\
        ("J_Na_SL_myo", 122), ("J_Ca_jct1_SL", 123), ("J_Ca_SL_myo", 124),\
        ("dh_dt", 125), ("dj_dt", 126), ("dm_dt", 127), ("dXr_dt", 128),\
        ("dXs_dt", 129), ("dX_tos_dt", 130), ("dY_tos_dt", 131),\
        ("dR_tos_dt", 132), ("dX_tof_dt", 133), ("dY_tof_dt", 134), ("dd_dt",\
        135), ("df_dt", 136), ("dfCaB_SL_dt", 137), ("dfCaB_jct1_dt", 138),\
        ("dR_dt", 139), ("dO_dt", 140), ("dI_dt", 141), ("dCa_TroponinC_dt",\
        142), ("dCa_TroponinC_Ca_Mg_dt", 143), ("dMg_TroponinC_Ca_Mg_dt",\
        144), ("dCa_Calmodulin_dt", 145), ("dCa_Myosin_dt", 146),\
        ("dMg_Myosin_dt", 147), ("dCa_SRB_dt", 148), ("dNa_jct1_buf_dt",\
        149), ("dNa_SL_buf_dt", 150), ("dNa_jct1_dt", 151), ("dNa_SL_dt",\
        152), ("dNai_dt", 153), ("dCa_Calsequestrin_dt", 154),\
        ("dCa_SLB_SL_dt", 155), ("dCa_SLB_jct1_dt", 156),\
        ("dCa_SLHigh_SL_dt", 157), ("dCa_SLHigh_jct1_dt", 158), ("dCa_SR_dt",\
        159), ("dCa_jct1_dt", 160), ("dCa_SL_dt", 161), ("dCai_dt", 162),\
        ("dV_dt", 163)])
...
def rhs(states, t, parameters, values=None):
    """
    Compute the right hand side of the shannon_2004_mouseIto ODE
    """
    # Imports
    import numpy as np
    import math
    ...
    X_toss = 1.0/(1 + math.exp(-3/13 - V/13))
    tau_X_tof = 0.08 + 0.7*math.exp(-((5/6 + V/30)*(5/6 + V/30)))
    values[8] = (-X_tof + X_toss)/tau_X_tof

    # Expressions for the Itof Y gate component
    Y_toss = 1.0/(1 + 14764.7815656*math.exp(V/5))
    tau_Y_tof = 10 + 32/(1 + 0.00097069329952*math.exp(-V/8)) + 8/(1 +\
        1808.04241446*math.exp(0.125*V))
    values[9] = (Y_toss - Y_tof)/tau_Y_tof
...
def monitor(states, t, parameters, monitored=None):
    """
    Computes monitored expressions of the shannon_2004_mouseIto ODE
    """
    # Imports
    import numpy as np
    import math
    ...
    # Expressions for the Itof X gate component
    monitored[47] = 1.0/(1 + math.exp(-3/13 - V/13))
    monitored[48] = 0.08 + 0.7*math.exp(-((5/6 + V/30)*(5/6 + V/30)))
    monitored[133] = (-X_tof + monitored[47])/monitored[48]

    # Expressions for the Itof Y gate component
    monitored[49] = 1.0/(1 + 14764.7815656*math.exp(V/5))
    monitored[50] = 10 + 32/(1 + 0.00097069329952*math.exp(-V/8)) + 8/(1 +\
        1808.04241446*math.exp(0.125*V))
